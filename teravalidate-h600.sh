...
if [ $EUID -eq 0 ]; then
   echo "this script should not be run as root. su to hdfs user to run"
   exit 1
fi
...
RESULTSFILE="./$LOGDIR/teravalidate_h600_results_$DATE"


OUTPUT=hdfs://hop-isi-s/data/teragen/${SIZE}-terasort-output
REPORT=hdfs://hop-isi-s/data/teragen/${SIZE}-terasort-report
